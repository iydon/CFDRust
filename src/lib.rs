// https://github.com/PyO3/rust-numpy/tree/v0.15.1/examples/simple-extension
use numpy::ndarray::{ArrayD, ArrayViewD, ArrayViewMutD};
use numpy::{c64, IntoPyArray, PyArrayDyn, PyReadonlyArrayDyn};
use pyo3::prelude::{pymodule, PyModule, PyResult, Python};

#[pymodule]
fn cfd_rust(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    // immutable example
    ...
    fn axpy_py<'py>(
        ...
        let x = x.as_array();
        let y = y.as_array();
        return axpy(a, x, y).into_pyarray(py);
    }
    ...
    fn conj_py<'py>(py: Python<'py>, x: PyReadonlyArrayDyn<'_, c64>) -> &'py PyArrayDyn<c64> {
        return conj(x.as_array()).into_pyarray(py);
    }
